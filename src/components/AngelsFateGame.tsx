...
export function AngelsFateGame() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const difficulty = Math.min(3, 1 + elapsed / 25);
      if (spawnTimerRef.current <= 0 && enemiesRef.current.length < Math.floor(1 + difficulty)) {
        ...
        enemiesRef.current.push({
          ...
          dir: fromRight ? -1 : 1,
          cooldown: 800 + Math.random() * 1200,
        });
        spawnTimerRef.current = 2500 / difficulty;
      }
      ...
      for (const e of enemiesRef.current) {
        ...
        if (e.cooldown <= 0) {
          ...
          const speed = 3 + difficulty * 0.4;
          fireballsRef.current.push({
            x: e.x + 30, y: e.y + 30,
            vx: (dx / len) * speed, vy: (dy / len) * speed,
            r: 8,
          });
          e.cooldown = 1200 + Math.random() * 1200;
        }
      }
      ...
      // ground
      ctx.fillStyle = "oklch(0.25 0.06 260)";
